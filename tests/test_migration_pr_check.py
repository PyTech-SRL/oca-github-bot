...
def test_no_new_module(mocker):
    ...
    """
    mocker.patch(f"{MIGRATION_PR_PATH}.github")
    mocker.patch(f"{MIGRATION_PR_PATH}.check_call")

    migration_issue = mocker.patch(f"{MIGRATION_PR_PATH}._find_issue")
    migration_issue.return_value.body = "Migration Issue Body"

    addons_gen = mocker.patch(f"{MIGRATION_PR_PATH}.addon_dirs_in")
...
def test_migration_comment(mocker):
    ...
    then it is a migration.
    """
    github_mock = mocker.patch(f"{MIGRATION_PR_PATH}.github")
    mocker.patch(f"{MIGRATION_PR_PATH}.check_call")
