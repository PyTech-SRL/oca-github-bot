...
def test_maintainer_mentioned(git_clone, mocker):
    ...
    repo_mock = github_mock.login.return_value.__enter__.return_value.repository
    repo_mock.return_value = gh_repo_mock
...
def test_mention_maintainer_other_branches(git_clone, mocker):
    ...
    def _file_contents(path, ref=None):
        subprocess.check_call(["git", "checkout", ref], cwd=git_clone)
        file_contents = mocker.MagicMock()
        file_contents.content = (git_clone / path).read_bytes()
        return file_contents
    ...
    with set_config(MAINTAINER_CHECK_ODOO_RELEASES=[maintainer_branch]):
        mention_maintainer("org", "repo", "pr")
